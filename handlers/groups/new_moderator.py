...
async def check_messages(message: types.Message):
    ...
    if message.html_text == '/info':
        ...
        if message.reply_to_message:
            if message.reply_to_message.chat.id == message.from_user.id:
...
async def logs_channel(message: types.Message):
    # Запись в логи
    async with aiofiles.open(file='logs_channel.txt', mode='a+', encoding='utf-8') as file:
        await file.write(
